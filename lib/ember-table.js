...
Ember.Table.TablesContainer = Ember.View.extend(Ember.StyleBindingsMixin, Ember.ResizeHandler, {
  ...
  forceFillColumns: function() {
    var additionWidthPerColumn, availableContentWidth, controller, defaultContentWidth, fixedColumnsWidth, numColumnToDistributeWidth, remainingWidth, tableColumns, totalWidth;
    ...
    if (defaultContentWidth < availableContentWidth) {
      ...
      return tableColumns.forEach(function(column) {
        if (column.get('canAutoResize')) {
          return column.incrementProperty('columnWidth', additionWidthPerColumn);
        }
      });
    }
  }
