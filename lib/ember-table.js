...
Ember.TouchHandlerMixin = Ember.Mixin.create({
  ...
  didInsertElement: function() {
    this._super();
    this.setupScroller();
    return this.updateScrollerDimensions();
  },
  updateScrollerDimensions: function() {
    var height, totalHeight, width;
    width = this.get('width');
    height = this.get('height');
    totalHeight = this.get('totalHeight');
    return this.get('scroller').setDimensions(width, height, width, totalHeight);
  },
  setupScroller: function() {
    var _this = this;
    return this.set('scroller', new Scroller(function(left, top, zoom) {
      if (_this.state !== 'inDOM') {
        return;
      }
      console.log(top);
      return _this.onTouchScroll(left, top, zoom);
    }));
  },
  touchStart: function(event) {
    event = event.originalEvent || event;
    return this.get('scroller').doTouchStart(event.touches, event.timeStamp);
  },
  touchMove: function(event) {
    event = event.originalEvent || event;
    return this.get('scroller').doTouchMove(event.touches, event.timeStamp);
  },
  touchEnd: function(event) {
    event = event.originalEvent || event;
    return this.get('scroller').doTouchEnd(event.timeStamp);
  }
});
...
Ember.Table.TableController = Ember.Controller.extend({
  ...
  _tableColumnsWidth: Ember.computed(function() {
    ...
    contentWidth = (this._getTotalWidth(this.get('tableColumns'))) + 26;
...
Ember.Table.TablesContainer = Ember.View.extend(Ember.StyleBindingsMixin, Ember.ResizeHandler, {
  ...
  onBodyContentDidChange: Ember.observer(function() {
  ...
  onResizeEnd: function() {
    this.elementSizeDidChange();
    return Ember.run.next(this, this.updateAntiscroll);
  },
  ...
  updateAntiscroll: function() {
    return this.$('.antiscroll-wrap').antiscroll();
  },
  updateLayout: function() {
    this.updateAntiscroll();
    if (this.get('controller.forceFillColumns')) {
      return this.forceFillColumns();
    }
  },
  forceFillColumns: function() {
    var availableContentWidth, columnWidth, contentPercentage, controller, defaultContentWidth, fixedAndContentWidth, fixedColumnWidth, fixedColumns, fixedColumnsWidth, newWidth, tableColumns, totalWidth;
    controller = this.get('controller');
    totalWidth = this.get('controller._width');
    fixedColumnsWidth = this.get('controller._fixedColumnsWidth');
...
Ember.Table.BodyTableContainer = Ember.Table.TableContainer.extend(Ember.MouseWheelHandlerMixin, Ember.ScrollHandlerMixin, Ember.TouchHandlerMixin, {
  ...
  onTouchScroll: function(left, top) {
    this.set('scrollTop', top);
    return this.set('scrollLeft', left);
  }
});
...
Ember.Table.ScrollContainer = Ember.View.extend(Ember.StyleBindingsMixin, Ember.ScrollHandlerMixin, {
  template: Ember.Handlebars.compile('<div class="antiscroll-wrap"><div class="antiscroll-inner">{{view Ember.Table.ScrollPanel}}</div></div>'),
